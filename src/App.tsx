...
import { LogOut, Menu, X, LayoutDashboard, History, Settings, AlertCircle } from 'lucide-react';
import { MockEchoLogo, MockEchoLogoWithWordmark } from './components/MockEchoLogo';
import { motion, AnimatePresence } from 'motion/react';

type OutletContextType = {
  onExitClick: (path: string) => void;
};
...
function LoginRoute() {
  ...
  return <AuthPage onBackToLanding={() => navigate('/')} />;
}
...
function AuthCallbackRoute() {
  ...
  return (
    <div className="min-h-screen bg-bg-warm flex flex-col items-center justify-center text-center px-4" id="oauth-callback-container">
      ...
      <p className="text-text-soft text-sm mt-2 max-w-md leading-relaxed">
        Redirecting to your interview dashboard...
      </p>
    </div>
  );
}
...
function DashboardRoute() {
  ...
  return (
    <Dashboard
      onStartNew={() => navigate('/interview/new')}
      onViewSession={(id) => navigate(`/interview/${id}/results`)}
      onViewHistory={() => navigate('/practice-logs')}
      onViewProfile={() => navigate('/settings')}
    />
  );
}

function CreateInterviewRouteWrapper() {
  const { onExitClick } = useOutletContext<OutletContextType>();
  const navigate = useNavigate();
...
function InterviewSessionRouteWrapper() {
  const { onExitClick } = useOutletContext<OutletContextType>();
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { activeSession, interviews, loading } = useApp();

  const sessionIsActive = activeSession && (id === 'active' || id === activeSession.id);

  useEffect(() => {
    if (id === 'active' && activeSession) {
      navigate(`/interview/${activeSession.id}`, { replace: true });
    }
  }, [id, activeSession, navigate]);

  if (sessionIsActive) {
    return (
      <InterviewSessionComponent
        onSessionFinished={(completedSession) => {
          const finishedId = completedSession?.id || activeSession?.id || id;
          navigate(`/interview/${finishedId}/results`);
        }}
        onCancel={() => onExitClick('/dashboard')}
      />
    );
  }
  ...
  const pastSession = interviews.find((x) => x.id === id);
  if (pastSession && pastSession.status === 'completed') {
    return <Navigate to={`/interview/${id}/results`} replace />;
  }

  return <Navigate to="/dashboard" replace />;
}

function ResultsViewRoute() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { interviews, activeSession } = useApp();

  const session = interviews.find((x) => x.id === id) || (activeSession?.id === id ? activeSession : null);
  ...
  return (
    <ResultsView
      ...
      onBackToHistory={() => navigate('/practice-logs')}
      onRetry={() => navigate('/interview/new')}
    />
  );
}

function PracticeLogsRoute() {
  const navigate = useNavigate();
...
function SettingsRoute() {
  ...
  return <ProfileView onBackToDashboard={() => navigate('/dashboard')} />;
}

// --- MAIN APP LAYOUT (FOR AUTHENTICATED ROUTES) ---

function AppLayout() {
  const { user, profile, activeSession, loading, error, profileError, retryLoadProfile, logout, cancelSession, demoMode } = useApp();
  const navigate = useNavigate();
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
          ...
          <div className="flex flex-col flex-1 min-h-0">
            ...
            <nav className="flex-1 py-6 space-y-1 overflow-y-auto">
              <button
                ...
                className={isCurrent('/dashboard') ? activeNavItemClass : inactiveNavItemClass}
              >
              ...
              <button
                onClick={() => handleNavClick('/practice-logs')}
                className={isCurrent('/practice-logs') ? activeNavItemClass : inactiveNavItemClass}
              >
              ...
              <button
                ...
                className={isCurrent('/settings') ? activeNavItemClass : inactiveNavItemClass}
              >
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            ...
            >
              ...
              >
                <LayoutDashboard className={`w-4 h-4 ${isCurrent('/dashboard') ? 'text-accent-forest' : 'text-text-soft'}`} />
                <span>Dashboard</span>
              </button>
              ...
              <button
                onClick={() => {
                  setMobileMenuOpen(false);
                  handleNavClick('/practice-logs');
                }}
                className={`w-full text-left py-3 px-4 rounded-xl font-bold text-sm flex items-center space-x-3 transition-colors ${
                  isCurrent('/practice-logs')
                    ? 'bg-accent-forest/10 text-accent-forest'
                    : 'text-text-soft hover:bg-bg-warm hover:text-text-charcoal'
                }`}
              >
                <History className={`w-4 h-4 ${isCurrent('/practice-logs') ? 'text-accent-forest' : 'text-text-soft'}`} />
                <span>Practice Logs</span>
              </button>
              ...
              >
                <Settings className={`w-4 h-4 ${isCurrent('/settings') ? 'text-accent-forest' : 'text-text-soft'}`} />
                <span>Preferences</span>
              </button>
        ...
        <main className="flex-1 py-8 px-4 sm:px-8 max-w-7xl mx-auto w-full overflow-y-auto">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Outlet context={{ onExitClick: handleNavClick }} />
            </motion.div>
          </AnimatePresence>
        </main>
...
function AppContent() {
  const { isRecoveryMode, setIsRecoveryMode, user } = useApp();
  const navigate = useNavigate();
  ...
  return (
    <Routes>
      ...
      <Route element={<AppLayout />}>
        ...
        <Route path="/interview/new" element={<CreateInterviewRouteWrapper />} />
        <Route path="/interview/:id" element={<InterviewSessionRouteWrapper />} />
        <Route path="/interview/:id/results" element={<ResultsViewRoute />} />
